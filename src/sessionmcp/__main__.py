...
1. Run the application as a module `uv run -m sessionmcp`
2. Run the application as a package `uv run sessionmcp`
...
# Import our enhanced SSE server components
...
def enhanced_main() -> None:
    """Start the client using asyncio with enhanced SSE server enabled by default."""
    # We're reusing most of the code from main() but forcing the enhanced flag to True
    parser = argparse.ArgumentParser(
        description=(
            "Start the MCP proxy with enhanced session parameter support."
        ),
        epilog=(
            "Examples:\n"
            "  sessionmcp-enhanced --sse-port 8080 -- your-command\n"
            "  sessionmcp-enhanced your-command --sse-port 8080 --allow-origin='*'\n"
        ),
    ...
    # Start a client connected to the given command, and expose as an enhanced SSE server
    logging.info("Starting stdio client and enhanced SSE server with session parameter support")
    ...
    # Always use the enhanced SSE server in this entry point
    ...
    asyncio.run(run_enhanced_sse_server(stdio_params, sse_settings))
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  sessionmcp http://localhost:8080/sse\n"
            "  sessionmcp --headers Authorization 'Bearer YOUR_TOKEN' http://localhost:8080/sse\n"
            "  sessionmcp --sse-port 8080 -- your-command --arg1 value1 --arg2 value2\n"
            "  sessionmcp your-command --sse-port 8080 -e KEY VALUE -e ANOTHER_KEY ANOTHER_VALUE\n"
            "  sessionmcp your-command --sse-port 8080 --allow-origin='*'\n"
            "  sessionmcp your-command --sse-port 8080 --enhanced  # Use the enhanced SSE server\n"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
    ...
    sse_server_group.add_argument(
        "--enhanced",
        action="store_true",
        help="Use the enhanced SSE server with session parameter support",
        default=False,
    )
    ...
    # If the enhanced flag is set, use the enhanced SSE server
    if args.enhanced:
        logging.info("Using enhanced SSE server with session parameter support")
        sse_settings = EnhancedSseServerSettings(
            bind_host=args.sse_host,
            port=args.sse_port,
            allow_origins=args.allow_origin if len(args.allow_origin) > 0 else None,
        ...
        # Otherwise use the original SSE server
...
def client() -> None:
    """Entry point for the mcp-reverse-proxy command."""
    # This is a placeholder for the future reverse proxy implementation
    print("mcp-reverse-proxy is not implemented yet")
    sys.exit(1)
